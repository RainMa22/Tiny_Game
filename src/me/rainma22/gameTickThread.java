    ...
    public void run() {
        ...
        while (true){
            ...
            while (currentTime-pastTime>=d){
                ...
                if (c.al.left){
                    c.playerCord.x -= 5;
                    if(c.playerCord.x+c.player.getWidth()/2<0)c.playerCord.x+=c.frame.getWidth();
                }
                if (c.al.right){
                    c.playerCord.x += 5;
                ...
                if (b==TICK_SPEED){
                    temp= (ArrayList<coords>) c.ebulletCords.clone();
                    for (coords cs:c.enemyCords){
                        temp.add(new coords(cs.x+c.enemy.getWidth()/2-c.ebullet.getWidth()/2,cs.y+c.enemy.getHeight()/2));
                    }
                    c.ebulletCords= (ArrayList<coords>) temp.clone();
                    b=0;
                }
